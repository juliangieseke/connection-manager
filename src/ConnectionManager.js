...
export default class ConnectionManager {
  ...
  constructor(maxConnections = 6, maxPriority = 4) {
    ...
    const context = {
      ...
      /**
       * @property {ConnectionQueue} tokens
       * @description internal token store
       * @name ConnectionManager~Context#tokens
       */
      ...
      setupConnection(connection) {
        connection.open(this.tokens.getHeader(connection));
      },
    ...
    this.storeToken = this.storeToken.bind(context);
  ...
  storeToken(token) {
    console.log(`ConnectionManager~storeToken: Storing the Token.`);
